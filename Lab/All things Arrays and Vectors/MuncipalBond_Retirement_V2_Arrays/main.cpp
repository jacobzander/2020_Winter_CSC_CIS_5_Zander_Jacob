...
int main(int argc, char** argv) {
    ...
    cout<<fixed<<setw(4)<<year<<setw(6)<<dYear
            <<setw(14)<<balance[0]<<setw(14)
            <<interest[0]<<setw(14)<<yrDep<<endl;
    
    //Loop for all years
    for(year++,dYear++;year<=nYears;year++,dYear++){
        balance[year]=balance[year-1]*(1+intRate);
        balance[year]+=yrDep;      //End of Year
        interest[year]=balance[year]*intRate;//End of Year
        cout<<setw(4)<<year<<setw(6)<<dYear
            <<setw(14)<<balance[year]<<setw(14)
            <<interest[year]<<setw(14)<<yrDep<<endl;
    }
    
    //Did you reach the goal
    if(balance[year - 1]>sav2Rtr)cout<<"Yes you reached your retirement goal"<<endl;
